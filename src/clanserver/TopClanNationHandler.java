...
public class TopClanNationHandler extends BaseClientRequestHandler{


	Object obj = null;
	public void handleClientRequest(User user, ISFSObject params) {
	
		trace("Sto richiedendo al server i 100 migliori clan per nazione");
		...
		try{
			...
			ISFSArray arr = dbmanager.executeQuery("SELECT guesswho.clan.*, "
					...
					+ "where guesswho.clan.position = ? group by guesswho.clan.clan_name "
					+ "order by guesswho.clan.trofei_total desc, guesswho.clan.clan_name limit 100 ", 
					new Object[] {nation});
			if (arr.size() > 0)
			{
			  SFSObject result = new SFSObject();
			  result.putSFSArray("success", arr);
			  send("topclansnation", result, user);
			}
			...
			send("topclansnation" , error, user);
	}
		}
